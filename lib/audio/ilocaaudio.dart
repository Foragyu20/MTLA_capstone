...
class AudioIlocanoList extends StatefulWidget {
  const AudioIlocanoList({super.key});

  @override
  AudioIlocanoListState createState() => AudioIlocanoListState();
}

class AudioIlocanoListState extends State<AudioIlocanoList> {
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        records = records.where((record) {
          return  record['ilocano'] != null && record['ilocano'].toUpperCase().contains(searchText.toUpperCase());
        }).toList();
      });
    } else {
      // Handle error
    ...
    Column(
      ...
      children: [
        Container(
          ...
          color: Comcol.db2,
          child:
          ListTile(title: Row(children: [const Text("Ilocano",style: TextStyle(fontSize: 40,fontFamily: "Kadwa",color: Comcol.dbl),), const SizedBox(width: 100,),
...
final player = AudioPlayer();
    await player.play(UrlSource(Api.playz+fileName));
  }
}